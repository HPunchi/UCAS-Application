...
//googleapis
/*
var resp = await createIPOPUP("Test", "Whats ur name>");
console.log(resp);
*/
...
var wallpaper = sessionStorage.getItem("wallpaper");
document.getElementById("mainBG").setAttribute("style", "background-image:url(" + wallpaper + ")");
//innerHTML
...
function seeEmojis(){
	...
	if ((document.getElementById("emoPicker").style.display == "none") || (document.getElementById("emoPicker").style.display == "")){
		...
		document.getElementById("chatPanelDIV").setAttribute("style", "bottom: 267.2px");

	
		document.getElementById("typePanelDIV").style.bottom = "207.2px";
		...
		document.getElementById("chatPanelDIV").setAttribute("style", "top:7.2px;bottom:70px;");
	
		document.getElementById("typePanelDIV").style.bottom = "7.2px";
		document.getElementById("typePanelDIV").style.right = "0px";
...
function closeIPopup(){
	iPopDATA = document.getElementById("iPopInput").value;
	iPopOpen = false;
	return true;
}	
async function createIPOPUP(title, prompt){
	var imgSrc = "/images/logo.png";
	document.getElementById("iPopTitle").innerText = title;
	document.getElementById("iPopImg").src = imgSrc;
	document.getElementById("iPopPrompt").innerText = prompt;
	document.getElementById("inputPOPUP").style.display = "block";
	document.getElementById("iPopInput").focus();
	iPopOpen = true;
	while (iPopOpen == true){
		await sleep(200);
	}
	document.getElementById("inputPOPUP").style.display = "none";
	document.getElementById("iPopInput").value = "";
	return iPopDATA;
...
async function sendMessage(){
	...
	if (dataFEEDBACK.status == "message-sent"){
		...
		newBtn.setAttribute("value", (messageArr.length - 1).toString());
...
function messageDetails(messageID){
	var messageName, messageTime, messageSender;
	for (var i=0;i<messageArr.length;i++){
		if (messageArr[i][3] == messageID){
			messageName = messageArr[i][0];
			messageTime = messageArr[i][2];
			messageSender = messageArr[i][1];
		}
	}
	var dStr = "Message: " + messageName + "\nSender: " + messageSender + "\nTime: " + messageTime;
	createPopup("Message Details", dStr);
	return;
}
...
async function openChat(userRec){
	...
	for (var i=0;i<messageArr.length;i++){
		...
		if (currentSender == "admin"){
			...
			newBtn.setAttribute("value", currentID);
			newBtn.setAttribute("onclick", "messageDetails(this.value)");
...
async function createNewChat(){
	var userRec = await createIPOPUP("Chat Creation", "Username of person you would like to chat with:");
...
async function viewAccountSettings(){
...
document.getElementById("textPOPUP").addEventListener("keydown", function(e){
	if (e.keyCode === 13){
		closePopup();
	}
});
document.getElementById("inputPOPUP").addEventListener("keydown", function(e){
	if (e.keyCode === 13){
		closeIPopup();
	}
});
